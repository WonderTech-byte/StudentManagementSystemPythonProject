...
class EnrollmentManager:
    def __init__(self, course_manager_object: CourseManager = None):
        if course_manager_object is None: self.course_manager = CourseManager()
        else: self.course_manager = course_manager_object

        self.__list_of_students = StudentManager().get_all_students()
        self.__list_of_enrollment: list[EnrolledCourse] = []

    def enroll_student(self, student_id: int, course_id: str) -> None:
        self.__check_course_exist(course_id)
        self.__check_duplicate_enrollment(student_id,course_id)
        if self.is_student_enrolled(student_id, course_id): raise ValueError("Student already enrolled for this course")
        self.__list_of_enrollment.append(EnrolledCourse(student_id, course_id))
    ...
    def is_student_enrolled(self, enrolled_id, course_id ) -> bool:
        for enrolled in self.__list_of_enrollment:
            if enrolled.get_student_id() == enrolled_id and enrolled.get_course_id() == course_id:
                return True
        return False
    ...
    def get_all_enrolled_course(self, student_id) -> list[str]:
        ...
        for enrolled in self.__list_of_enrollment:
            if enrolled.get_student_id() == student_id:
                course_list.append(enrolled.get_course_id())
        return course_list
    ...
    def __check_duplicate_enrollment(self, student_id, course_id):
        for enrollment in self.__list_of_enrollment:
            if enrollment.get_student_id() == student_id and enrollment.get_course_id()==course_id:
    ...
    def get_enrolled_course(self, student_id, course_id):
        for enrolled in self.__list_of_enrollment:
            if enrolled.get_student_id() == student_id and enrolled.get_course_id() == course_id:
                return enrolled
